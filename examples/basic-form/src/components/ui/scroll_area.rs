...
impl ScrollArea {
    /// Create a new scroll area with the given id.
...
impl RenderOnce for ScrollArea {
    fn render(self, _window: &mut Window, _cx: &mut App) -> impl IntoElement {
        let mut el = div().id(self.id).relative();
...
mod tests {
    ...
    fn test_scroll_area_horizontal() {
        let area = ScrollArea::new("test").orientation(ScrollOrientation::Horizontal);
