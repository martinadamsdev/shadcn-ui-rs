...
//!
//! A horizontal range slider that displays a numeric value within a range.
//!
//! **Note:** This component is currently display-only. It renders the track,
//! filled portion, and thumb at the correct position based on the current
//! `value`, but does not handle click or drag interaction. GPUI's layout
//! model does not easily expose element-relative click positions needed for
//! calculating a new value from user input. To make the slider interactive,
//! you would need to implement a custom GPUI element with hit-testing and
//! drag handling.
//!
//! # Example
//! ```rust
